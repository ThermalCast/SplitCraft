  ...
  async function generatePlanWithAI({ goal, daysPerWeek, equipment, notes, repRangeMin, repRangeMax, splitType, fixedSets }) {
    ...
    const notesInstruction = notes
      ? `\nHEALTH CONSTRAINT — read this first; it overrides every other instruction in this prompt, including the pinned/required-exercise lists below AND the requested split structure: "${notes}"\n`
        ...
        + `Do not let the requested split shape override this: if a conventional day for that split (e.g. a "Legs" day) would normally center on an exercise this constraint rules out, do NOT include it anyway to keep the day's theme intact. Fill that day with safe alternatives that train the same muscles without the risky movement pattern instead (e.g. leg curls, hip thrusts, leg extensions, glute bridges, hip abduction/adduction machine work for a lower-body day, none of which load the injured area the way a standing loaded squat or lunge does), and only give that day fewer exercises than usual if no safe alternative exists in the provided list. Prefer seated, machine-based, or non-weight-bearing alternatives for the affected area generally, and drop a pinned or required exercise entirely rather than violate this constraint.\n`
      : '';

  ...
  async function generatePlanWithAI({ goal, daysPerWeek, equipment, notes, repRangeMin, repRangeMax, splitType, fixedSets }) {
    ...
    // "Previous plan" means previously TRAINED, not merely previously
    // GENERATED. The most recently created plan is normally the one this
    // call is about to replace — including one just generated minutes ago,
    // disliked, and regenerated without a single set ever logged against it
    // (exactly what testing this feature looks like). Steering the model
    // away from a plan nobody actually did isn't a real variety signal, so a
    // plan only counts as "previous" here once it has at least one logged
    ...
    logPlanStatus(previousPlan
      ...
      : existingPlans.length > 0
        ? `${existingPlans.length} previous plan record(s) exist but none has a logged set yet — skipping the variety comparison.`
    ...
    // instructions below when the two collide. This app's own exercise data
    // only tags PRIMARY-MOVER muscles for volume tracking too (Back Squat is
    // tagged quads/glutes, never calves), so it can't be used to judge injury
    // safety the way it's used elsewhere in this prompt for pinned exercises.
    //
    // A first version of this instruction spelled out a specific worked
    // example (a calf strain ruling out standing squats/lunges) to make the
    // "not just the directly-trained muscle" point concrete. Deliberately
    // genericized back out: a hardcoded example is pattern-matching on one
    // injury, not a general fix, and this is being tested without it to see
    // whether the general instruction alone is enough for a capable model.
    // If it isn't, the real fix is a verification pass over the generated
    // plan (a second call checking output against the stated constraint),
    // not a bigger pile of injury-specific examples here.
    const notesInstruction = notes
      ? `\nHEALTH CONSTRAINT — read this first; it overrides every other instruction in this prompt, including the pinned/required-exercise lists below AND the requested split structure: "${notes}"\n`
        + `If this describes an injury or physical limitation, exclude every exercise that loads, stabilizes through, or bears weight through the affected area — not only exercises that directly train it. This app's own exercise data tags each exercise's PRIMARY and visible secondary muscles for volume tracking, not every stabilizer or joint involved, so don't rely on it to judge safety here — reason about it directly. When unsure whether a movement is safe, leave it out.\n`
        + `Do not let the requested split shape override this: if a conventional day for that split (e.g. a "Legs" day) would normally center on an exercise this constraint rules out, do NOT include it anyway to keep the day's theme intact. Fill that day with safe alternatives that train the same muscles without the risky movement pattern instead, and only give that day fewer exercises than usual if no safe alternative exists in the provided list. Prefer seated, machine-based, or non-weight-bearing alternatives for the affected area generally, and drop a pinned or required exercise entirely rather than violate this constraint.\n`
      : '';
...
${schemaExample}`;
    ...
    for (const day of parsed.days) {
      ...
      // DISLIKED IS ENFORCED HERE, NOT JUST REQUESTED IN THE PROMPT.
      //
      // Disliked exercises are dropped from the candidate list the model is
      // shown, but "you MUST choose exclusively from this list" is still
      // just an instruction — a model that decides an excluded exercise
      // belongs anyway (this is exactly how a health-constraint note can
      // fail: the model judges a movement safe/appropriate and includes it
      // regardless of what it was told to avoid) will name it, and byName
      // above is deliberately built from EVERY exercise so that name still
      // resolves to the existing disliked record instead of spawning a
      // duplicate custom one. That match is the hook needed to catch and
      // drop it here, after generation, rather than only hoping the model
      // never says the name — the one place in this function that isn't
      // purely prompt-and-trust.
      const before = day.exercises.length;
      day.exercises = day.exercises.filter(e => !dislikedIds.has(e.exerciseId));
      if (day.exercises.length < before) {
        logPlanStatus(`Removed ${before - day.exercises.length} exercise(s) from "${day.name || 'a day'}" that are marked disliked — enforced regardless of what the model chose.`);
      }
    }
